...
- **A project somebody shared with you opens in Lazuli.** They send you the folder's name — their Syncing dialog has it ready to copy — and you paste it into **Add shared…** on the launch screen; Google's own file chooser opens on that folder, and choosing it is what lets Lazuli read it. It is an ordinary project from then on, entries they add later included. A project Lazuli syncs is kept in a `Lazuli` folder in your Drive and named `Lazuli | <project>`, which is both tidier and what makes it findable among everything else anyone has ever shared with you.
