...
- **Your name and picture no longer vanish.** Two things erased them. Any older version of Lazuli that saved its settings wrote back only the settings it knew, which took your name and picture with it — so running an older copy beside a newer one kept wiping them. And when the settings file could not be read for a moment, the app took it for a blank file and saved the blankness back. Who you are now lives in a file of its own that no older version knows about, a file that cannot be read is left exactly as it is, and the same goes for your list of projects and tabs.
- **A newer version's changes survive an older one editing the same project.** Every file Lazuli rewrites — a project, an entry, an author, the trash, your project list — now keeps whatever it does not recognise, so a project shared between people on different versions no longer loses what the newer one wrote each time the older one saves.
- **Your name can differ in one project.** Open the round button in the corner while the project is on screen, and set what you are called there; leave it empty and your own name shows, and renaming yourself later reaches every project you have not overridden.
- **Your other devices know you are you.** The first time a device opens a project whose authors it has not met, it asks which of them is you. Choose yourself and that device writes under your name and picture from then on, so an entry written on one computer and one written on another are by the same person rather than by two people who happen to share a name.
- **Keep a project in Dropbox, OneDrive, iCloud, Drive for Desktop, Syncthing or git, and share it that way.** Lazuli does not sync anything itself; everything above is what makes it safe to let something else do it. The README says how.
- **An entry that came back from a merge in two versions says so, instead of disappearing.** Conflict markers in an `entry.md` made the file unreadable, and Lazuli quietly left that entry off the timeline — nothing was lost on disk, and nothing told you. Such an entry now sits on its own day with both versions side by side and a button under each; the one you do not keep goes to the project's trash rather than away. The second copy a folder syncer leaves beside an entry, rather than touching it, is offered the same way — whichever syncer it was, Syncthing, Dropbox, OneDrive, iCloud or another, since each names its copy differently. Nothing is ever merged for you: two versions of a sentence is a question only you can answer.
