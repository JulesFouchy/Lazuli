...
- **Entries record who wrote them, and say so once more than one person has.** A project kept somewhere two people can both write to — a shared folder, or a repository — now shows a name beside the date on each entry. A journal you keep by yourself looks exactly as it did: the name appears only when a project has more than one author. Your own name starts as your account name on this machine. Entries written before this are left as they are rather than being claimed for anyone.
