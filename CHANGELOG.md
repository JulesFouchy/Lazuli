...
- **Add shared… opens Google's folder chooser, and the project does not arrive yet.** Choosing the folder hands Lazuli the folder and none of the files inside it — that is how far Google's `drive.file` permission reaches, and Lazuli now says so plainly instead of telling you that you chose the wrong folder. Receiving a project somebody shared needs a wider permission from Google, which is a decision still being made. A project Lazuli syncs is kept in a `Lazuli` folder in your Drive and named `Lazuli | <project>`, and the owner's Syncing dialog has that name ready to copy.
