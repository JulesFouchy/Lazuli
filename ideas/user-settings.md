...
- **It is global, not per-project.** It describes when *you* sleep, not anything about a project. It must not go in `lazuli.yaml`, or the same entry would land on different days in different projects.
...
The date format was here too, and went the other way: it is a property of the project, so it lives in `lazuli.yaml`. Ask of anything else on this list whether the same is true of it before moving it here.
