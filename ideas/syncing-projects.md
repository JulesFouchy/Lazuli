...
- **The Picker**, which is how somebody else's project gets in, through Google's *desktop* flow: the sign-in redirect with `trigger_onepick=true` and `allow_folder_selection=true`, answered on the loopback the app already listens on. Choosing the folder *is* the grant, which is why a pasted link cannot stand in for it and why the `drive` scope — restricted, and an annual security assessment — stays turned down. The web flow was built first and removed: it needs an API key, a fixed origin, the project number and the browser's Google cookies in a `docs.google.com` frame, and when any of that is missing it takes the choice, spins, and silently drops it.
