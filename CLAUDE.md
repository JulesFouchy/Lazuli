...
- **The sync engine writes files and stops there.** `sync.rs` reconciles a project against a [`Backend`] — four operations over a dumb store — and the watcher's rescan is what puts the result on screen. Nothing in the app has to know a sync happened, and `sync.rs` knows nothing about Tauri, an account or a clock, which is what makes the whole decision table testable against an in-memory remote.
- **`.lazuli/` is this machine's, and never syncs.** The sync base — what the two sides last agreed on, per file — is what makes reconciling a decision rather than a guess, and another machine's copy of it would be a lie here. `.lazuli-trash/` and `.lazuli-thumbs/` *do* sync: a deletion is a change like any other, and it is the only way the other machine learns about one.
- **An author is recognised by account, not by machine.** An author id lives in this machine's settings, so nothing carries it to the next one. What crosses is the `accounts:` list in a project's `authors/<id>/profile.yaml`: a second device looks its own Google account up there and *joins* that author rather than minting a rival, which is the difference between one person and one-per-machine. `display_name` overrides the name in that project alone, and republishing the global name never overwrites it.
- **Drive enforces who may do what, and Lazuli draws it.** Reader, writer and owner are Google's, checked on Google's servers. There is no access control here to write, and a members view is a screen over `permissions.list`. "May edit only their own entries" is not enforceable against a text editor and is deliberately not pretended at.
